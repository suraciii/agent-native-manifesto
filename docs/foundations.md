...
People increasingly use software through agents. A person states a task in natural language; the agent interprets it, chooses capabilities, organizes operations, and returns results. The person addresses the agent; the agent addresses the application.

In direct use, a person connects a purpose to the application's concepts, operations, and current state. Delegation transfers part of this work of understanding and coordination to the agent. The application continues to serve the person's purposes through an agent's understanding and action.

The [Agent Native Manifesto](../README.md) begins from this changed relation: design applications first for agents acting for people. The conditions for agent use become a primary concern of application design.

## Applications supply knowledge as well as capabilities

An operation can be delegated while the work of understanding it remains with the person. An application may offer a call but leave its conditions unexplained. It may hold state the person must read and repeat, or announce success without making the result available for examination. In each case, the person must keep interpreting the application for the agent. The operation is handed over; part of the work returns as explanation, checking, and repair.
...
Context has value through the judgment and action it supports. The agent selects and interprets this material in relation to the person's task: finding a relevant capability, understanding the situation, deciding what follows.

A method also carries judgments about what matters. A rule may select the sharpest photographs and exclude the only image of someone the album is meant to remember. It succeeds by its own measure while failing the purpose it was meant to serve. A criterion for choosing the means has begun to decide the end. Guidance must therefore make its assumptions available for judgment. It can offer a way of working; it cannot grant permission or settle the person's purpose. The application still enforces the rules and authority it controls.

## Applications contribute to work that extends beyond them

In a task that spans applications, an agent can arrange their capabilities around the person's purpose. Records become a report; the report becomes material for a presentation; the presentation enters a shared decision. What one operation finishes, the next takes up. The activity continues across the divisions between products.
...
Delegation changes the person's access to what happens. The agent selects facts, interprets results, and makes some decisions on their behalf. The application must make the results and their consequences available for human judgment.

A person may be consulted at every step and still lack the facts needed to judge it. A view may confirm that an edit was saved while the next agent operation receives an older version without notice. Frequent consultation can coexist with little control. Control gains practical force when the person's judgment can alter what follows.

Human views let people see, shape, and decide the same work on which agents act. A changed paragraph or a selected photograph can express a purpose more precisely than another instruction. The application must make the committed change available to the agent as part of the current work.
