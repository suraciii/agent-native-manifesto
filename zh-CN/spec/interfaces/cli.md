...
对于失败、部分效果或结果未知的情况，通过有文档说明的通道提供结构化诊断记录。将记录关联到操作、工作、条目或输出，说明已知效果、已有的原因或依赖信息、不确定性，以及下一次受支持的检查或恢复操作。说明诊断是否存在延迟、采样、截断或不完整。面向人的文字可以补充解释，但不应成为 Agent 唯一的恢复路径。诊断输出不得泄露秘密。
...
| 诊断 | 效果产生后的失败；依赖故障；批次部分完成；延迟或不完整的诊断记录；敏感输出边界 |
