...
通过操作结果、Problem Details、工作状态资源或有范围的诊断资源提供诊断事实。每项事实都应关联到相关操作、工作、对象或修订版本。区分观察到的事实、应用的推测和未知情况。提供已知效果、相关依赖条件、诊断信息的范围与时效性，以及下一次受支持的检查或恢复操作。

根据调用方权限、目标以及时间或工作范围限制诊断查询。说明延迟、采样、脱敏、截断和保留限制。原始日志或链路追踪可以提供细节，但不应成为 Agent 唯一的恢复路径。不要求追踪协议或单独的遥测服务。
...
对于异步工作，保留状态资源。客户端可以通过通知、有界等待或程序轮询获取更新，无需让模型决定每一次查询。对于流和 webhook，定义访问、交付和刷新行为，并在相关时说明顺序和重复处理。除非契约另有说明，请求超时不会取消远程工作。
...
将这些链接视为敏感信息。需要长期引用时，使用对象标识与经过授权的获取方式。支持此能力的访问路径可以通过不透明句柄，将访问材料保留在模型上下文之外。大型产出物可以使用独立获取路径，避免在每次响应中重复内容。
...
| 诊断 | 效果产生后的失败；依赖超时；部分结果；延迟或不完整的诊断数据；未获授权的诊断访问 |
