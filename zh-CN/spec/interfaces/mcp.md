...
MCP 将宿主连接到服务器的工具、资源和提示模板。本指南应用[核心要求](../core.md)；另见[接口概览](../interfaces.md)。

示例与来源使用 **MCP 2025-11-25**。评估时，应以应用和宿主实际支持的协议修订版本为准。

## 作用与适用条件

宿主必须支持所需的传输、授权和能力。MCP 不建立全局产品发现：仍需由用户、注册表、管理员或宿主提供服务器入口。
...
在所引用的协议修订版本中，工具由模型控制、资源由应用驱动、提示模板由用户控制，这是其设计模式。宿主的呈现方式可能不同；列出资源并不意味着已将其加载进模型。
...
工具注解描述预期行为；目录条目不会授权调用方对所有对象执行操作。即使宿主缓存了目录，执行时仍要检查当前权限和对象条件。
...
资源 URI 不一定能直接获取。服务器端的 `file` 资源未必位于 Agent 本地。在每个受支持的宿主中检查资源获取和界面交接。

测试目录分页与更新；在每个宿主中度量工具选择和上下文使用。不要假设宿主会按需发现工具或注入整个目录；即使目录很小，含糊的描述也可能掩盖能力。
...
本地传输可以从宿主环境接收凭证。这不意味着授予无限制的文件系统或网络权限；仍须执行适用范围。如果宿主无法完成所需访问流程，应提供受支持的交接，而不是让模型在任务内容中携带秘密。
...
| 上下文与宿主行为 | 宿主提供工具但不支持假定的资源路径；过期目录；界面扩展不可用；大型结果与有限上下文 |

使用[评估流程](../evaluation.md)，包括在每个声称支持的宿主中执行任务。
