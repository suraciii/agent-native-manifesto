...
在基于模型的 Agent 中，这种关系由模型的输入中介。模型不能直接接触应用内部的能力。它只能依据应用放入输入的内容判断操作：接口描述、schema、流程 Skills、当前事实、工具结果和反馈。一个可调用但无法从现有输入中被理解和选择的操作，在技术上存在，在 Agent 的实际工作中却等于不存在。

应用不能控制整份输入，也不能保证模型一定理解。它只对描述自身能力、状态、条件、效果和限制的那部分负责。这些材料可以通过初始指令、检索、工具调用或后续结果进入输入。不必一次性全部加载，但需要在需要时真实、当前、可用并可取得。
...
应用已经掌握 Agent 所需的一部分知识：操作意味着什么、在什么条件下可用、会改变什么，以及工作处于什么状态。提供使用能力所需的知识，本就是提供能力的一部分。对于基于模型的 Agent，这些知识只有能够进入模型输入时才会真正发挥作用。应用不只是暴露可调用接口，还要提供让该接口进入 Agent 判断的材料。

应用不负责整份输入，也不负责保证模型一定作出正确选择。它负责以宿主或 Agent 能够放入输入的形式，提供自身的使用条件。这些形式可以是接口描述、流程 Skills、当前状态或工具结果；这项责任不绑定于某一种格式。
...
## 面向人的界面不应成为 Agent 的门槛

Agent Native 并不意味着移除人的界面。它意味着，面向人的操作路径不应成为使用应用能力的前提。如果 Agent 必须浏览页面、从布局推断含义或模拟点击，应用要求它重演人的路径，而不是直接提供 Agent 的使用路径。翻译工作因此又回到 Agent，并在失败时回到用户。

面向人的视图有另一项必要作用：展示结果与证据，让用户检查、编辑、决定、调整方向或接管同一项工作。当工作需要视觉判断时，视图可以丰富甚至复杂。复杂性不是问题；问题是应用的导航成为能力和状态唯一可用的地方。Agent 的操作和人的视图应作用于同一领域状态，使已提交的人类修改进入后续的模型输入和 Agent 操作。
...
Agent 有时可以适应原本为人设计的操作路径，从而使用应用。成功可能体现的是 Agent 的适应能力，而非应用对此类使用方式的支持。Agent Native 将 Agent 使用所需的条件确立为产品自身的责任。这包括理解和选择能力所需的模型输入，但不要求移除人的界面或采用特定协议。
