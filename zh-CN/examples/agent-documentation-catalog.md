...
| catalog 条目 | 链接到的材料 | 目的 |
| --- | --- | --- |
| 产品概览 | 支持的工作、结果、提供方、限制和条件 | 在详细使用前判断是否适合 |
| Agent 快速开始 | 安装或连接、身份、访问权限和首次使用路径 | 不依赖隐藏的设置知识开始使用 |
| 任务指南 | 目标、依赖、选择、结果检查和恢复 | 遵循推荐路径或进行调整 |
| 接口契约 | 带版本的 CLI、HTTP、MCP、SDK 或文件契约 | 正确选择和调用能力 |
| 人的参与 | 决定、负责角色、交接和直接操作 | 保留人的主导和权限边界 |
| 诊断与恢复 | 状态、观察到的效果、未知情况、依赖和下一步操作 | 在失败或结果不确定后继续工作 |
| 限制与生命周期 | 预算、保留、删除、导出、撤销和进行中工作的限制 | 限定工作及其后果 |
...
- catalog 不安装工具，也不建立连接。
- catalog 不验证调用方身份，也不授予权限。
- 凭证、令牌和特定用户的私有数据不应放入 catalog。
- catalog 中的链接不是操作成功的证据。
- 操作契约描述行为；应用代码仍然负责验证输入、强制执行领域不变量和权限约束，并产生效果。
- 如果发布了多套文档，catalog 应说明版本或范围差异。

## 评估

将 catalog 作为声明的发现和说明路径的一部分进行评估：

- 从正常的发现环境或正常产品入口开始。
- 记录 Agent 找到并使用了哪些 catalog 条目。
- 检查链接是否指向当前、准确且可访问的材料。
- 测试一项常见任务、一项边界情况和一次可恢复失败。
- 记录不必要的查找、缺失的依赖、误导性描述和额外的人为解释。
